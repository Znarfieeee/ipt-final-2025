...
// Add a bypass verification schema
const bypassVerificationSchema = Joi.object({
    email: Joi.string().email().required(),
})
...
// Add route for bypassing verification
router.post(
    "/bypass-verification",
    validateSchema(bypassVerificationSchema),
    bypassVerification
)
...
function bypassVerification(req, res, next) {
    accountService
        .bypassVerification(req.body.email)
        .then(result => res.json(result))
        .catch(next)
}
