...
async function getAll() {
  try {
    console.log("Starting to fetch all users in service layer");
    ...
    console.log(`Found ${accounts.length} users in database`);
    ...
    try {
      console.log("Returning fallback admin user data");
      const adminUser = {
        id: 1,
        firstName: "Admin",
        lastName: "User",
...
async function sendVerificationEmail(account, origin) {
  ...
  console.log(`Verification email would be sent to ${account.email}`);
  return true;
}

async function sendPasswordResetEmail(account, origin) {
  // In a real implementation, you would send an actual email
  console.log(`Password reset email would be sent to ${account.email}`);
  return true;
}
